...
class _DiscoverScreenState extends State<DiscoverScreen> {
  ...
  bool _isGeneratingStories = false;
  bool _hasAutoGenerated = false;

  List<Article> _filteredArticlesOf(MediaLibraryService mediaLibraryService) {
    final stories = mediaLibraryService.generatedStories;
    if (_selectedLevel == 'All') return stories;
    return stories.where((a) => a.level == _selectedLevel).toList();
  }

  /// Generates a fresh batch of level-tagged reading passages via the user's
  /// own Gemini key (BYOK — same pattern as the transcript cleanup pass), and
  /// adds them to the catalog. No-ops quietly on the very first auto-trigger
  /// if no key is configured; explicit taps tell the user why nothing happened.
  Future<void> _generateStories(
    BuildContext context,
    MediaLibraryService mediaLibraryService, {
    bool silent = false,
  }) async {
    if (_isGeneratingStories) return;
    final apiKey = await GeminiApiKeyStore.getKey();
    if (apiKey == null) {
      if (!silent && context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Add a Gemini API key in Settings to generate stories.'),
    ...
    setState(() => _isGeneratingStories = true);
    if (!silent && context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Generating new stories...'), duration: Duration(seconds: 3)),
      );
    }

    List<String> focusWords = const [];
    try {
      final dueWords = await VocabularyService().getDueWords();
      focusWords = dueWords.take(5).map((w) => w.baseForm ?? w.word).toList();
    } catch (_) {}

    final avoidTitles = mediaLibraryService.generatedStories.map((a) => a.title).take(20).toList();

    try {
      final stories = await StoryGenerationService.generateStories(
        apiKey: apiKey,
      ...
      if (mounted) setState(() => _isGeneratingStories = false);
    }
  }
  ...
  void initState() {
    super.initState();
    // First-run convenience: if the catalog is empty and the user already has
    // a Gemini key configured, generate an initial batch automatically rather
    // than showing a dead end. Silent — no snackbar noise, and it simply does
    // nothing if there's no key yet (the empty-state card's button covers that).
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted || _hasAutoGenerated) return;
      _hasAutoGenerated = true;
      final mediaLibraryService = Provider.of<MediaLibraryService>(context, listen: false);
      if (mediaLibraryService.generatedStories.isEmpty) {
        _generateStories(context, mediaLibraryService, silent: true);
      }
    });
  }
          ...
          Expanded(
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        _buildGenerateStoriesButton(context, mediaLibraryService),
                      ],
                    ),
                  ],
                ),
                ...
                if (_filteredArticlesOf(mediaLibraryService).isEmpty)
                  _buildEmptyStoriesState(context, mediaLibraryService)
                else
                  ResponsiveGrid(
                    ...
                    expandedColumns: 2,
                    largeColumns: 3,
                    itemCount: _filteredArticlesOf(mediaLibraryService).length,
                    itemBuilder: (context, index) {
                      final article = _filteredArticlesOf(mediaLibraryService)[index];
                      return ArticleCard(
                        article: article,
  ...
  Widget _buildGenerateStoriesButton(BuildContext context, MediaLibraryService mediaLibraryService) {
    final colorScheme = Theme.of(context).colorScheme;
    return Tooltip(
      message: 'Generate new stories',
      child: InkWell(
        onTap: _isGeneratingStories ? null : () => _generateStories(context, mediaLibraryService),
        borderRadius: BorderRadius.circular(4),
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(4),
            border: Border.all(color: colorScheme.primary.withValues(alpha: 0.3), width: 0.8),
          ),
          child: _isGeneratingStories
              ? SizedBox(
                  width: 16,
                  height: 16,
  ...
  Widget _buildEmptyStoriesState(BuildContext context, MediaLibraryService mediaLibraryService) {
    final colorScheme = Theme.of(context).colorScheme;
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            _selectedLevel == 'All'
                ? 'No stories yet.'
                : 'No $_selectedLevel stories yet.',
            style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14),
          ),
          const SizedBox(height: 4),
          Text(
            'Generate a fresh, diverse batch of German reading passages using your Gemini key.',
            style: TextStyle(fontSize: 12, color: colorScheme.onSurfaceVariant),
          ),
          const SizedBox(height: 12),
          FilledButton.icon(
            onPressed: _isGeneratingStories ? null : () => _generateStories(context, mediaLibraryService),
            icon: _isGeneratingStories
                ? const SizedBox(
                    width: 14,
                    height: 14,
            ...
            label: Text(_isGeneratingStories ? 'Generating...' : 'Generate Stories'),
          ),
        ],
      ),
    );
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: _filteredArticlesOf(mediaLibraryService).isEmpty
                ? _buildEmptyStoriesState(context, mediaLibraryService)
                : ListView.separated(
                    shrinkWrap: true,
