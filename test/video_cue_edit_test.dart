...
void main() {
  ...
  testWidgets(
    'editing cue shows floating window that can be dragged and repositioned',
    (tester) async {
      SharedPreferences.setMockInitialValues({});
      ...
      final video = ProcessedVideo(
        id: 'video-drag-1',
        taskId: 'task-drag-1',
        url: 'https://example.com/video-drag',
        status: ProcessingStatus.completed,
        subtitles: [initialCue],
      );
      ...
      // Open Edit Dialog
      final cueWidget = find.textContaining('Hallo').first;
      await tester.longPress(cueWidget);
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 300));
      ...
      // Find drag handle icon and verify floating window presence
      final dragIcon = find.byIcon(Icons.drag_indicator_rounded);
      expect(dragIcon, findsOneWidget);

      final initialTopLeft = tester.getTopLeft(find.byKey(const Key('edit_cue_original_field')));

      // Drag floating window by 50px right, 60px down
      await tester.drag(dragIcon, const Offset(50, 60));
      await tester.pump();

      final newTopLeft = tester.getTopLeft(find.byKey(const Key('edit_cue_original_field')));
      expect(newTopLeft.dx, greaterThan(initialTopLeft.dx));
      expect(newTopLeft.dy, greaterThan(initialTopLeft.dy));
